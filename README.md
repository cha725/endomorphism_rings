...
$$\mathrm{End}_A(A \oplus \mathrm{rad}^i(A)  \mid  i > 0)$$
...
To compute $$\mathrm{End}_A(A \oplus \mathrm{rad}^i(A)  \mid  i > 0)$$, we find all module homomorphisms between these graph representations.
